...
describe('defaults', () => {
    ...
    test('should use GET headers', () => {
        const X_CUSTOM_HEADER = 'X-CUSTOM-HEADER';
        axios.defaults.headers.get[X_CUSTOM_HEADER] = 'foo';
        axios.get('/foo');
        return getAjaxRequest().then(request => {
            expect(request.requestHeaders[X_CUSTOM_HEADER]).toBe('foo');
            delete axios.defaults.headers.get[X_CUSTOM_HEADER];
        });
    });
    test('should use GET headers', () => {
        const X_CUSTOM_HEADER = 'X-CUSTOM-HEADER';
        axios.defaults.headers.post[X_CUSTOM_HEADER] = 'foo';
        axios.post('/foo');
        return getAjaxRequest().then(request => {
            expect(request.requestHeaders[X_CUSTOM_HEADER]).toBe('foo');
            delete axios.defaults.headers.post[X_CUSTOM_HEADER];
        });
    });
    test('should use header config', () => {
        const instance = axios.create({
            headers: {
                ...
                get: {
                    'X-GET-HEADER': 'getHdeaderValue',
                },
                post: {
                    'X-POST-HEADER': 'postHdeaderValue',
                },
            },
        });
        instance.get('/foo', {
